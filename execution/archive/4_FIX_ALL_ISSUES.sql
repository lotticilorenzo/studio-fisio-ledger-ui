-- ============================================================
-- STUDIO FISIO LEDGER - SCRIPT DI RIPRISTINO COMPLETO (FIX ALL)
-- ============================================================
-- Esegui questo script interamente nel SQL Editor di Supabase.
-- Risolve:
-- 1. Errore "admin_create_appointment" mancante
-- 2. Errore colonna "duration_minutes" mancante
-- 3. Tutte le altre RPC mancanti (op_create, admin_summary, ecc)
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- 1. AGGIUNTA COLONNA duration_minutes (se manca)
-- ------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (
...
-- ------------------------------------------------------------
-- 2. RPC: admin_create_appointment
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 3. ALTRE RPC MANCANTI (Op Functions + Admin Reports)
-- ------------------------------------------------------------
...
RETURNS TABLE (
  operator_id uuid, operator_name text, num_appointments bigint,
  total_gross_cents bigint, total_commission_cents bigint, total_net_cents bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    op.id, op.display_name, count(a.id),
...
-- admin_link_user_to_operator
CREATE OR REPLACE FUNCTION admin_link_user_to_operator(
  p_user_email text, p_display_name text, p_commission_rate numeric
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
